...
class Graf:
    ...
    def valideaza (self):
        ...
        matrDesfasurata = sum (self.start, [])
        if sorted(matrDesfasurata) != list(range(9)):
            return False
        
        nrInversiuni = 0
        for index1, element1 in enumerate(matrDesfasurata):
            for index2, element2 in enumerate(matrDesfasurata[index1+1:]):
                if element1 != 0 and element2 != 0 and element1 > element2:
                    nrInversiuni += 1
